 ...
 *
 * Deliberately not part of `scripts/release-gate.mjs`. The gate verifies the
 * artifact by reading it, needs no dependencies, and so cannot itself be broken
 * by a dependency change. This check has to install a real dependency tree, so
 * it lives in its own script and its own CI job, and the gate keeps that
 * property.
 *
 * Usage:
 *   node scripts/entry-load.mjs <tarball> [--react 18,19] [--keep]
 *
 * Why this earns its place next to the gate's static checks: it catches the
 * #759 crash by observing the failure rather than by pattern-matching the
 * artifact. Verified against the published tarballs: 4.2.5 throws
 * "Calling `require` for \"react\" in an environment that doesn't expose the
 * `require` function" on the ESM entry while its CJS entry loads fine, and
 * 4.2.6 loads on both. `exports-map` cannot see this: every file it checks for
 * is present in 4.2.5, it just does not run.
 */
...
function main() {
  ...
  try {
    ...
    console.error('  the files are all present, they just do not run. Check that no');
    console.error('  CJS module was inlined into the ESM build (npm run gate).');
    return 1;
  } finally {
